...
static const char      board_type[] = "q8";                           /* type of board controlling the motor */
...
static void
encoder_counts_to_theta(const t_int32 counts[MOTORS])
{
    static const double factor = 2 * PI / 20000;

    int i;
    double theta = counts[i] * factor;
}
...
int main(int argc, char* argv[])
{
    ...
    printf("This example controls a single motor at %g Hz.\n", frequency);

    /* Open the board that controls the motor */
    ...
    if (result == 0)
    {
        ...
        {
            struct limiter_state current_limiter = { 0 };
            double desired_torque = 0.5; /* Example desired torque in N-m */

            while (stop == 0)
            {
                /* Drive the motor to produce the desired torque */
                generate_torque(board, &current_limiter, period, desired_torque);

                /* Prepare for the next sampling instant */
                // No sleep function is needed; timing is handled by the Quanser API
            }

            stop_controller(board, task);

            printf("\nController has been stopped. Press Enter to continue.\n");
            (void)getchar(); /* absorb Ctrl-C */
        }
